...
> เวอร์ชันล่าสุด: **v3.11.1** · ไม่รู้ว่าโปรเจกต์ใช้รุ่นไหน → `node buaflow/bin/buaflow.js lock` (ถ้ามี `.buaflow/lock.json`)
> หรือดูแถวสัญญาณใน [START-HERE.md](START-HERE.md) ข้อ 2.1
...
|---|---|---|
| **v2.3.4 – v3.11.0** | [ทางลัด: ไป v3.11.1 ในรอบเดียว](#fast-path) | ~10 นาที + ขั้นที่ต้องลงมือถ้าเข้าเงื่อนไข |
...
## ทางลัด: จาก v2.3.4 หรือ 3.x รุ่นไหนก็ได้ → v3.11.1 ในรอบเดียว

ทำได้เพราะทุกรุ่นหลัง v3.0.0 เป็น MINOR/PATCH ([release policy](standards/release-policy.md)): ของเดิมทำงานเหมือนเดิม
และตัวตรวจใหม่ทำงาน**เฉพาะเมื่อโปรเจกต์มีไฟล์หลักฐานนั้น** ⇒ การอัปเกรดคือ "คัดลอกไฟล์ควบคุมชุดล่าสุดทับ" บวกขั้นที่ต้องลงมือ
...
วาง `buaflow/` เวอร์ชันใหม่ทับของเดิม · **ใช้ `git clone`/`git pull` ดีที่สุด** เพราะข้อ 2 ใช้ประวัติ git ของ kit แยกไฟล์รุ่นเก่าออกจากไฟล์ที่ทีมแก้เอง

### 2. หาว่าไฟล์ไหนทีมแก้เอง — ก่อนคัดลอกทับ

```bash
# ยังไม่มี .buaflow/lock.json (ใช้รุ่นก่อน 3.11): ถ่ายภาพไฟล์ที่ติดตั้งอยู่ตอนนี้ก่อน
node buaflow/bin/buaflow.js lock --write

node buaflow/bin/buaflow.js lock
```

| สถานะ | แปลว่า | ทำอะไร |
|---|---|---|
| `current` | ตรงกับ kit ล่าสุดแล้ว | ไม่ต้องทำอะไร |
| `outdated` | เป็นไฟล์ของ kit รุ่นเก่า (lock หาเจอในประวัติ git ของ kit) | คัดลอกทับได้เลย |
| `customized` / `drifted` | ไม่เคยเป็นไฟล์ของ kit รุ่นไหน = **ทีมแก้เอง** | จดไว้ ข้อ 3 จะทับ แล้วค่อยนำการแก้กลับมาใส่ |

> ถ้า `buaflow/` ไม่ใช่ git checkout lock มองประวัติไม่เห็น ไฟล์ที่ต่างจาก kit ทุกไฟล์จะขึ้น `customized` —
> ดูว่าไฟล์ไหนทีมแก้จริงด้วย `git log --oneline -- .claude/<ไฟล์>` ของโปรเจกต์

### 3. คัดลอกไฟล์ควบคุมชุดล่าสุด

```bash
for f in buaflow/claude-setup/*.js; do
  case "$(basename "$f")" in
    assess.js|benchmark.js|intake.js|kit-lock.js|local-ci.js) ;;   # รันจาก kit ไม่ต้องคัดลอก
    *) cp "$f" .claude/ ;;
  esac
done
cp buaflow/claude-setup/hooks/*.js   .claude/hooks/
cp buaflow/claude-setup/agents/*.md  .claude/agents/
cp -r buaflow/claude-setup/skills/*  .claude/skills/
mkdir -p docs/templates && cp -n buaflow/templates/*.tpl.* docs/templates/   # -n: ไม่ทับ template ที่ทีมปรับแล้ว
grep -qx '.verify-flakes.jsonl' .gitignore || echo .verify-flakes.jsonl >> .gitignore
```

**ไม่คัดลอกทับ — ของเหล่านี้เป็นของโปรเจกต์:** `.claude/stack.json`, `.claude/settings.json`, `.claude/rules/`
ให้เปิดเทียบกับ `buaflow/claude-setup/` แล้วยกมาเฉพาะที่เกี่ยว:
...
แล้วนำการแก้ของทีมจากข้อ 2 กลับมาใส่ในไฟล์ที่ถูกทับ (`git diff` ของไฟล์นั้นคือสิ่งที่หายไป)
...
| ติดตั้ง Buaflow เป็น plugin ด้วย | ลบ block `hooks` ออกจาก `.claude/settings.json` ไม่งั้น hook รันสองรอบ | [claude-plugin/README.md](claude-plugin/README.md) |

ไม่มีข้อไหนตรง = ข้ามข้อนี้ทั้งข้อ

### 5. บันทึกเวอร์ชันแล้วตรวจ

```bash
node buaflow/bin/buaflow.js lock --write     # ยอมรับชุดใหม่ (รวมการแก้ของทีมที่ใส่กลับแล้ว)
