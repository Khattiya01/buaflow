...
// works — that a dump taken from this sync backend's database can be destroyed and restored, and that the
...
//
// The backup and restore themselves run through `docker compose exec`, so no Postgres client is
// needed on the host. Only the roll-forward step needs the backend's Python environment; set
// PYTHON to point at a specific interpreter if it is not the active one.
...
async function main() {
  ...
  const before = await step("fingerprint the database before backup", () => {
    ...
    if (!value.rows.users) throw new Error("no rows in users — run this rehearsal against a seeded database");
  ...
  const backup = await step("take a backup with pg_dump", () => {
    ...
    if (CONTAINER) run("docker", ["cp", `${CONTAINER}:${DUMP_IN_CONTAINER}`, dumpOnHost]);
    else run("docker", ["compose", "cp", `db:${DUMP_IN_CONTAINER}`, dumpOnHost]);
  ...
  const rollForward = await step("roll forward after restore, as the runbook instructs", () => {
    run(PYTHON, ["-m", "alembic", "upgrade", "head"], { cwd: backend, stdio: ["ignore", "pipe", "pipe"] });
    const value = fingerprint();
    ...
    return { noPendingMigrations: true, dataUnchanged: true, ok: true };
  });
