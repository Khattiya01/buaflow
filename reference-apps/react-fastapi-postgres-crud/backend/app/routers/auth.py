...
def login(body: LoginRequest, response: Response, db: Session = Depends(get_db)) -> UserOut:
    ...
    # Same generic error whether the account is missing or the password is wrong — do not
    # let the API reveal which accounts exist (basic security-controls hygiene).
    if user is None or not verify_password(password, user.password_hash):
        # Never log the email on failure — logging "which emails were tried" turns logs
        # into an account-enumeration oracle for anyone who can read them.
