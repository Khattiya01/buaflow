...
// browser driving the real UI against the real production build and a real database — no
// mocks.
test("a member can sign in, add a task, and see it in their list", async ({ page }) => {
  ...
  // Delete removes it from the visible list (soft delete — proven at the API layer in
...
test("signing out blocks access to /tasks again", async ({ page }) => {
  ...
  // Unlike PP-003's Next.js middleware (which denies at the edge before any HTML ships),
  // this SPA's guard runs client-side after the page briefly loads and calls the API —
  // documented as a known difference in docs/security-notes.md.
