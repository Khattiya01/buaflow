...
// against a freshly created (no prior volume) Postgres, apply migrations, seed, and prove the
// primary flow works — all from a torn-down starting state, none of it reusing a warm cache or an
// already-running app process. Run: node scripts/rehearse-clean-environment.mjs
//
// Note: this proves "from-scratch build + boot" against the current working tree. It does not
// prove a fresh `git clone` — that additionally requires the tree to be committed and pushed,
// which this script does not do. See evidence/clean-environment-rehearsal.json's `scope` field.
...
const IMAGE = "nextjs-postgres-crud:rehearsal";
const CONTAINER = "nextjs-postgres-crud-rehearsal";
const PORT = 3199;
const NETWORK = "nextjs-postgres-crud_default";
...
async function main() {
  ...
  await step("tear down any existing compose stack (including volumes)", () => {
    spawnSync("docker", ["compose", "down", "-v"], { cwd: root, shell: true });
  });
  ...
  await step("wait for Postgres to accept connections", async () => {
    ...
    while (Date.now() < deadline) {
      const res = spawnSync("docker", ["compose", "exec", "-T", "db", "pg_isready", "-U", "app", "-d", "app"], {
        cwd: root,
        shell: true,
      });
  ...
  await step("apply migrations to the fresh database", () => {
    sh("npx", ["prisma", "migrate", "deploy"], { env: { ...process.env } });
  });
  ...
  await step("remove any leftover rehearsal container", () => {
    spawnSync("docker", ["rm", "-f", CONTAINER], { cwd: root, shell: true });
  });
  ...
  await step("smoke test: log in as the seeded admin and list tasks", async () => {
    ...
    const tasksRes = await fetch(`http://localhost:${PORT}/api/tasks`, { headers: { cookie: cookie ?? "" } });
    if (!tasksRes.ok) throw new Error(`task list smoke test failed: ${tasksRes.status}`);
