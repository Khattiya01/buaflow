...
| BC-008 | Parallel tasks stop colliding on the same files | 🔄 **in progress (D-030)** — ไม่ใช่ scheduler (BC-003 ยัง dropped): task ประกาศ `touches:` · `/spec` ติด `[P]` เฉพาะ task ที่ไฟล์ไม่ซ้ำ · `/task` เทียบกับ PR ที่เปิดอยู่ · `/check` `/done` เอา main เข้า branch ก่อน · docs-lint warn · เปิดจากโปรเจกต์จริงที่ PR ชนกันบ่อยเพราะ task ที่แตกจาก spec เดียวแก้ไฟล์เดียวกัน |
