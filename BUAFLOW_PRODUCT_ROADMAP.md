...
| PE-012 | Kit files stay verbatim | ✅ **done** (3.19.0) — `install` ทับเงียบ ๆ เฉพาะเนื้อหาที่ kit เคยส่ง (lock ที่รับ format ของ Prettier ไม่พอ → conflict) · โปรเจกต์ที่ใช้ Prettier ได้ไฟล์ของ kit ใน `.prettierignore` · `doctor` เตือน · จาก trendy |
...
| EV-011 | เก็บข้อมูลการใช้งานภายใน (opt-in) | **done (D-028)** — ส่งใน kit 3.13.0 (+ 3.13.1 แก้โปรเจกต์ `"type": "module"`): ยินยอมใน `/buaflow:start` · hook `usage-capture` · sync ไป private repo · `buaflow usage report/show/eval-draft` · ใช้จริงแล้วใน Bluepeak Hub และ TRENDY · เปิดใหม่แทน EV-005 เพราะมีผู้ใช้จริงแล้ว (Bluepeak Hub + โปรเจกต์ภายใน 2–5 โปรเจกต์) · ถามยินยอมครั้งเดียวต่อโปรเจกต์ · เก็บ event ตาม lifecycle พร้อมชื่อ model และเนื้อหาเต็ม · รวมที่ private git repo · ใช้ทำ eval corpus, เทียบผลของ model, dashboard รวม และเคสอ้างอิง |
