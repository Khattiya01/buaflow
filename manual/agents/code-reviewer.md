...
# code-reviewer — delegated role, done serially

> Reviews the branch diff against the plan, the constitution, and project standards — the things the built-in /code-review cannot know. Called by /check after the code is written and before merge.
...
- **model-tiers** — There is one model. Ignore the tier; the playbook works the same, only slower or more expensive.
